...
- Строка версии показывает `v.2.1 (последняя)` или `v.2.1 (доступна v.2.2)`.
...
Если установлена последняя версия, меню показывает `v.2.1 (последняя)`.
Если доступно обновление, эта же строка меняется на `v.2.1 (доступна v.2.2)`
и становится единственной кнопкой обновления.

**Windows:** в меню появляется строка `v.2.1 (доступна v.2.2)` → нажимаешь →
скрипт скачивает и применяет обновление автоматически.

**macOS:** в попапе появляется строка `v.2.1 (доступна v.2.2)` → нажимаешь → запускается
