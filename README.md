...
Если установлена последняя версия, меню показывает `v.2.0 (последняя)`.
Если доступно обновление, эта же строка меняется на `v.2.0 (доступна v.2.1)`
и становится единственной кнопкой обновления.

**Windows:** в меню появляется строка `v.2.0 (доступна v.2.1)` → нажимаешь →
скрипт скачивает и применяет обновление автоматически.

**macOS:** в попапе появляется строка `v.2.0 (доступна v.2.1)` → нажимаешь → запускается
